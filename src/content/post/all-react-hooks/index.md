...
هوک‌ها (Hooks) یکی از مهم‌ترین قابلیت‌های React هستند که از نسخه 16.8 معرفی شدند و امکان استفاده از State، مدیریت چرخه حیات کامپوننت‌ها و بسیاری از قابلیت‌های دیگر را در کامپوننت‌های تابعی فراهم می‌کنند. پیش از معرفی هوک‌ها، توسعه‌دهندگان برای استفاده از این امکانات مجبور به استفاده از Class Componentها بودند، اما امروزه هوک‌ها به استاندارد اصلی توسعه در React تبدیل شده‌اند.
در این آموزش، تمامی هوک‌های React را به‌صورت جامع بررسی می‌کنیم؛ از هوک‌های پایه مانند "useState" و "useEffect" گرفته تا هوک‌های پیشرفته‌تر مانند "useMemo"، "useCallback"، "useReducer" و سایر هوک‌های کاربردی. همچنین با مثال‌های عملی، نحوه استفاده صحیح از هر هوک، موارد استفاده و نکات مهم مربوط به عملکرد و بهینه‌سازی را یاد خواهید گرفت.
...
هوک useEffect برای اجرای کدهای جانبی (Side Effects) در کامپوننت‌های تابعی استفاده می‌شود. کارهایی مانند دریافت داده از API، کار با تایمرها، افزودن Event Listener یا تغییر مستقیم DOM معمولاً داخل useEffect انجام می‌شوند.
...
```
آرایه وابستگی (Dependency Array) مشخص می‌کند که Effect چه زمانی اجرا شود؛ آرایه خالی ([]) باعث می‌شود فقط یک‌بار پس از رندر اولیه اجرا شود.
...
هوک useLayoutEffect مشابه useEffect است، با این تفاوت که بلافاصله بعد از اعمال تغییرات روی DOM و قبل از نمایش آن به کاربر اجرا می‌شود. از این هوک زمانی استفاده می‌شود که نیاز به اندازه‌گیری یا تغییر عناصر DOM قبل از رندر شدن صفحه داشته باشید تا از پرش یا چشمک زدن رابط کاربری جلوگیری شود.
...
هوک useRef برای نگهداری یک مقدار قابل تغییر بین رندرهای مختلف کامپوننت استفاده می‌شود. تغییر مقدار ref باعث رندر مجدد کامپوننت نمی‌شود و معمولاً برای دسترسی مستقیم به عناصر DOM یا ذخیره مقادیر موقتی به کار می‌رود.
...
هوک useMemo برای ذخیره کردن نتیجه‌ی یک محاسبه استفاده می‌شود تا در رندرهای بعدی، تا زمانی که وابستگی‌ها تغییر نکرده‌اند، محاسبه دوباره انجام نشود. این کار باعث بهبود عملکرد برنامه در محاسبات سنگین می‌شود.
...
```
در مثال بالا، heavyCalculation فقط زمانی اجرا می‌شود که data تغییر کند.
...
هوک useCallback برای ذخیره (Memoize) کردن یک تابع بین رندرهای مختلف کامپوننت استفاده می‌شود. این هوک باعث می‌شود تا زمانی که وابستگی‌ها (Dependencies) تغییر نکرده‌اند، همان نمونه قبلی تابع مجدداً استفاده شود و تابع جدید ساخته نشود.

کاربرد اصلی: جلوگیری از رندرهای غیرضروری کامپوننت‌های فرزند و بهبود عملکرد در سناریوهایی که توابع به‌عنوان prop ارسال می‌شوند.
```
const handleClick = useCallback(() => {
  console.log("Clicked!");
}, []);
```
در مثال بالا، تابع handleClick فقط یک بار ساخته می‌شود و در رندرهای بعدی مجدداً ایجاد نخواهد شد، مگر اینکه مقادیر موجود در آرایه وابستگی تغییر کنند.
...
هوک useContext برای دسترسی به داده‌های اشتراکی (Global State) در یک Context استفاده می‌شود. این هوک به کامپوننت‌ها اجازه می‌دهد بدون نیاز به ارسال props در چندین سطح، به داده‌ها و توابع موردنیاز دسترسی داشته باشند و آن‌ها را مصرف کنند.
...
فهمیدن React Hook ها و موارد استفاده‌ی مناسبشون برای ساختن اپلیکیشن‌های React کارآمد و قابل نگه‌داری خیلی مهمه. در حالی که بعضی از Hook ها مثل `useState` و `useRef` همراهان روزانه‌ هستند بقیه اهداف خاصی دارن که کمتر بهشون برمی‌خوری.
